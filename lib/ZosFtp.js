...
module.exports = (config) => {
  ...
  async function put (source, hostFile, optionsCopy) {
    ...
    if (hostFileType === 'pds') {
      ...
      if (!(await pathExists(pdsDataSet))) await execCommand(`zowe zos-files create data-set-partitioned ${pdsDataSet}`, config)
    }
    ...
    if (hostFileType === 'dataset') {
      ...
      await execCommand(`zowe zos-files create data-set-sequential "${hostFile}" ${extraOptions}`, config)
    }

    await execCommand(`zowe zos-files upload file-to-data-set "${sourceFilePath}" "${hostFile}" --encoding ${encoding}`, config)
  ...
  async function get (hostFile, localFilePath) {
    ...
    await execCommand(`zowe zos-files download data-set "${hostFile}" --file ${finalLocalFilePath} --encoding ${encoding}`, config)
  ...
  async function del (hostFile) {
    ...
    await execCommand(`zowe zos-files delete data-set "${hostFile}" --for-sure`, config)
  }

  async function list (path) {
    const executionResult = await execCommand(`zowe zos-files list data-set "${path}"`, config)
